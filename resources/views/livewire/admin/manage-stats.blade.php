<div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ $sport === 'soccer' ? 'Matches' : ($sport === 'track' ? 'Meets' : 'Games') }}</th>
    ...
    <div class="bg-white shadow rounded-lg p-6" 
        ...
        <h3 class="text-lg font-medium text-gray-900 mb-4">
            {{ $editingId === 'new' ? 'Add' : 'Edit' }} Season Stats
        </h3>
        ...
        <form wire:submit.prevent="save" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    @error('season_year') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">{{ $sport === 'soccer' ? 'Matches' : ($sport === 'track' ? 'Meets' : 'Games') }} *</label>
                    <input type="number" wire:model="competitions" class="w-full px-3 py-2 border border-gray-300 rounded-md">
                    @error('competitions') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
            </div>

            <div class="border-t border-gray-200 pt-4">
                <h4 class="text-md font-medium text-gray-900 mb-4">{{ ucfirst($sport) }} Statistics</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($statFields as $field => $value)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                {{ ucwords(str_replace('_', ' ', $field)) }}
                            </label>
                            @if(in_array($field, ['best_time', 'personal_record']))
                                <input type="text" 
                                       wire:model="statFields.{{ $field }}" 
