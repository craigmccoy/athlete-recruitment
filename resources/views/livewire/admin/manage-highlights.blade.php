<div>
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden flex flex-col">
            <div class="aspect-video bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center relative overflow-hidden">
                ...
                <div class="absolute top-2 right-2 z-10 flex flex-col items-end gap-2">
                    <span class="text-xs px-2 py-1 rounded font-semibold {{ $highlight->type === 'video' ? 'bg-purple-600 text-white' : 'bg-green-600 text-white' }}">
                        {{ ucfirst($highlight->type) }}
                    </span>
                ...
                @if($highlight->type === 'photo' && $highlight->photo_path)
                    @php
                ...
                @elseif($highlight->type === 'video' && $highlight->video_url)
                    @php
                        // Get video thumbnail
            ...
            <div class="p-4 flex flex-col flex-grow">
                <div class="flex-grow">
                    ...
                    @if($highlight->type === 'video' && $highlight->duration)
                    <p class="text-xs text-gray-500">Duration: {{ $highlight->duration }}</p>
                    @endif
                </div>
                
                <!-- Edit/Delete Actions - Bottom Right -->
                <div class="flex justify-end space-x-2 mt-4 pt-3 border-t border-gray-200">
                    <button wire:click="edit({{ $highlight->id }})" 
                            wire:loading.attr="disabled"
                            wire:target="edit({{ $highlight->id }})"
                            class="text-blue-600 hover:text-blue-900 text-sm disabled:opacity-50 inline-flex items-center gap-1">
                        <svg wire:loading wire:target="edit({{ $highlight->id }})" class="animate-spin h-3 w-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        Edit
                    </button>
                    <button wire:click="delete({{ $highlight->id }})" 
                            wire:loading.attr="disabled"
                            wire:target="delete({{ $highlight->id }})"
                            onclick="return confirm('Are you sure?')" 
                            class="text-red-600 hover:text-red-900 text-sm disabled:opacity-50 inline-flex items-center gap-1">
                        <svg wire:loading wire:target="delete({{ $highlight->id }})" class="animate-spin h-3 w-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        Delete
    ...
    <div class="bg-white shadow rounded-lg p-6" 
        ...
        <h3 class="text-lg font-medium text-gray-900 mb-4">
            {{ $editingId === 'new' ? 'Add' : 'Edit' }} Highlight
        </h3>
                ...
                @else
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Photo {{ $editingId === 'new' ? '*' : '(Optional - leave empty to keep current)' }}
                        </label>
                        ...
                        @if ($photo)
                            <div class="mt-2">
                                <img src="{{ $photo->temporaryUrl() }}" class="h-32 rounded border">
                            </div>
                        @elseif($editingId !== 'new')
                            ...
                            @if($highlight && $highlight->photo_path)
                                <div class="mt-2">
                                    ...
                                    <img src="{{ $currentPhotoUrl }}" class="h-32 rounded border">
                                </div>
                            @endif
                        @endif
                    </div>
